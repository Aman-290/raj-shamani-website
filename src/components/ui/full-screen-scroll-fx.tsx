    ...
    const handleLoadedStagger = () => {
      leftItemRefs.current.forEach((el, i) => {
        gsap.fromTo(
          el,
          { opacity: 0, y: 20 },
          { opacity: i === index ? 1 : 0.2, y: 0, duration: 0.5, delay: i * 0.06, ease: "power3.out" }
        );
      });
      rightItemRefs.current.forEach((el, i) => {
        gsap.fromTo(
          el,
          { opacity: 0, y: 20 },
          { opacity: i === index ? 1 : 0.2, y: 0, duration: 0.5, delay: 0.2 + i * 0.06, ease: "power3.out" }
        );
      });
    };
    ...
    return (
      ...
      >
                    ...
                    {s.renderBackground ? (
                      ...
                      <>
                        <img
                          ...
                          src={s.background}
                          alt=""
                          className="fx-bg-img mix-blend-overlay opacity-50"
                        />
                    ...
                    {sections.map((s, sIdx) => {
                      ...
                      return (
                        <div key={`C-${s.id ?? sIdx}`} className={`fx-featured ${sIdx === index ? "active" : ""}`}>
                          <h3 className="fx-featured-title bg-clip-text text-transparent bg-gradient-to-r from-white to-[#d8b068]">
                  ...
                  {showProgress && (
                    <div className="fx-progress">
                      <div className="fx-progress-numbers">
                        <span ref={currentNumberRef} className="text-[#d8b068] font-bold">{String(index + 1).padStart(2, "0")}</span>
        ...
        <style jsx>{`
          ...
          .fx-left-item.active, .fx-right-item.active { opacity: 1; color: var(--fx-text); }
          .fx-left-item.active { transform: translateX(10px); padding-left: 16px; border-left: 2px solid #d8b068; }
          .fx-right-item.active { transform: translateX(-10px); padding-right: 16px; border-right: 2px solid #d8b068; text-align: right; }
          ...
          .fx-progress { width: 250px; height: 2px; margin: 1rem auto 0; background: rgba(255,255,255,0.1); position: relative; }
          .fx-progress-fill { position: absolute; inset: 0 auto 0 0; width: 0%; background: #d8b068; height: 100%; transition: width 0.3s ease; }
          .fx-progress-numbers { position: absolute; inset: auto 0 100% 0; display: flex; justify-content: space-between; font-size: 0.8rem; color: var(--fx-text); padding-bottom: 8px;}
